cutDendro<-function(df,cutG,dendroZoom) {
    ...
    if (dbg.dendro.cut) printVar(selectedClusterCandidateCount)
    if (selectedClusterCandidateCount>1) {
        ...
        if (dbg.dendro.cut>1) printVar(parentClusters)
        selectedClusters<-rep(NA,selectedClusterCandidateCount)
        selectedClustersIdx<-0
        #if (dbg.dendro.cut) printVar(currentYlim)
        ...
        if (dbg.dendro.cut>1) printVar(selectedClusters)

        selectedClusterCount<-0
        unselectedIndices<-1:df$clusterCount
        for (i in mySeq(1,selectedClustersIdx)) {
            if (dbg.dendro.cut>1) printVar(i)
            if (dbg.dendro.cut>1) printVar(selectedClusters[i])
            if (!is.na(selectedClusters[i]) && selectedClusters[i]>0) {
                ...
                if (dbg.dendro.cut>1) printVar(subclusters)
                tmp<-clusterId2SegmentIds(subclusters)
                if (dbg.dendro.cut>1) printVar(tmp)
        ...
        if (dbg.dendro.cut>1) printVar(df$clusters)
